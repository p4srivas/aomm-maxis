...
helm install --namespace nokia-aomm postgres-aomm \
...
kubectl get secret --namespace nokia-aomm postgres-aomm -o jsonpath="{.data.postgres-password}" | base64 -d
...
metadata:
  name: aomm-configmap
data:
  DB_HOST: "10.255.8.206"
  DB_PORT: "5432"
  ROOT_PATH: "/usr/src/app/"
  POSTGRES_PWD: "Klccr055Ot"
EOF
...
spec:
  template:
    spec:
      containers:
      - name: aomm-setup
        image: p4srivas/aomm-setup:v1.0012
...
#GKE     http://34.170.167.219 environment 
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: aomm-fastapi
        image: p4srivas/fastapi-aomm:v1.0109f
        ports:
        - containerPort: 8000
        env:
        - name: DB_HOST
...
kubectl --namespace aomm delete deployment aomm-fastapi

kubectl --namespace aomm scale --replicas=0 deployment/aomm-fastapi
kubectl --namespace aomm scale --replicas=1 deployment/aomm-fastapi
...
kubectl --namespace aomm delete service aomm-fastapi
...
http://prod.ccsmlops.he-pi-os-ohn-005.k8s.dyn.nesc.nokia.net:32600/ environment
...
kubectl --namespace nokia-aomm delete deployment nokia-aomm

kubectl --namespace nokia-aomm logs nokia-aomm

kubectl --namespace nokia-aomm delete deployment nokia-aomm

kubectl --namespace nokia-aomm scale --replicas=0 deployment/nokia-aomm
kubectl --namespace nokia-aomm scale --replicas=1 deployment/nokia-aomm
...
kubectl --namespace nokia-aomm delete service nokia-aomm
